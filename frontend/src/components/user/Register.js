...
const Register = () => {
    ...
    const signUpForm = () => (
        <form onSubmit={handleSubmit}>
            ...
            <button type="submit" className="btn btn-success" disabled={disabled}>Create Account</button>
        </form>
    );
    ...
    const showSuccess = () => {
        if (success) return (
            <div className="alert alert-primary">
                New Account Created. Please <Link style={{ color: '#00704A'}} to="/login">Login</Link>.
            </div>
        )
    }
    ...
    return (
        <Layout title="Register" className="container col-md-8 offset-md-2">
            ...
            <h3 style={{ color: '#00704A'}}>Register Here</h3>
            <hr />
            {signUpForm()}
            <hr />
        </Layout>
